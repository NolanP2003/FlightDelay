...
<head>
    ...
    <style>
        ...
        td.high-delay { color: #dc3545; font-weight: bold; }    /* Red */
        td.moderate-delay { color: #ffc107; font-weight: bold; } /* Yellow */
        td.no-delay { color: #28a745; font-weight: bold; }     /* Green */
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                {% set prob = doc.get('Probability_Severe_Delay', 0) %}
                {% if prob < 0.3 %}
                    <td class="no-delay">No Delay Predicted</td>
                {% elif prob < 0.6 %}
                    <td class="moderate-delay">Moderate Chance for Delay</td>
                {% else %}
                    <td class="high-delay">High Chance for Delay</td>
                {% endif %}
                <td class="probability">{{ "{:.2f}".format(prob * 100) }}</td>
            </tr>
            {% endfor %}
        </tbody>
    </table>
